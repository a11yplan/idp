...
import { useSession, admin, organization } from "@/lib/auth-client"
...
export default function AdminUserDetailPage() {
    ...
    const fetchUserData = async () => {
      try {
        ...
        if (usersResult.data?.users) {
          const foundUser = (usersResult.data.users as any[]).find((u: any) => u.id === userId)
          if (foundUser) {
            setUser(foundUser)
        ...
        // Note: We'll need to implement this using organization queries
  ...
  const handleBanUser = async () => {
    ...
    try {
      ...
      setUser((prev) => prev ? { ...prev, banned: true, banReason } : null)
  ...
  const handleUnbanUser = async () => {
    try {
      ...
      setUser((prev) => prev ? { ...prev, banned: false, banReason: null } : null)
    } catch (error: any) {
